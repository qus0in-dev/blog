...
Astro 기반 개인 기술 블로그입니다. 콘텐츠는 Markdown/MDX로 관리하고, 공개 글은 Cloudflare Workers Assets 대상으로 빌드 및 배포합니다. 조회수는 D1에 저장하고, 광고 노출은 환경 변수로 켜고 끌 수 있습니다.
...
- `PUBLIC_ENABLE_KAKAO_AD`
  ...
  - 현재 운영 기본값은 [`.env.production`](./.env.production) 기준 `false`입니다.
...
- `wrangler deploy` 대상은 [wrangler.jsonc](./wrangler.jsonc) 기준 `@astrojs/cloudflare/entrypoints/server` 입니다.
...
- `gh-open-pr-browser`: 현재 브랜치를 푸시하고 PR 제목과 짧은 본문 초안을 채운 뒤 브라우저에서 마무리
...
- `repo-skill-authoring`: 로컬 스킬을 `SKILL.md`, `agents/openai.yaml`, `references/` 구조로 맞춰 작성
